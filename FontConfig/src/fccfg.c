...
/* Return an array of paths to search for the fonts.conf file.
 */
static FcChar8 **
FcConfigGetPath (void)
{
    ...
    npath = 2;	/* by default only return 1 dir + null */

    /* Incr npath if additional paths available in FONTCONFIG_PATH */
    ...
    /* alloc array for pointers to path */
    ...
    /* copy paths from FONTCONFIG_PATH, if present */
    if (env)
    {
	   e = env;
	   while (*e)
