...
void calculate_extents(box* b, hb_glyph_info_t glyph_info, hb_glyph_position_t glyph_pos, FT_Face ft_face, double point_size) {
  ...
  FT_BBox ft_bbox;
  FT_Fixed advance;
  const FT_Glyph_Metrics *ftmetrics;

  error =  FT_Load_Glyph(ft_face, glyph_info.codepoint, FT_LOAD_NO_SCALE);
  if (error) 
    return;
  ...
  ftmetrics = &ft_face->glyph->metrics;
...
__declspec(dllexport) int __cdecl face_from_options(lua_State* L) {
  FT_Face face;
  FcChar8 * font_path;
  FcPattern* p;
  FcPattern* matched;
  ...
  font_path = (FcChar8 *)_strdup((const char *)font_path);
  if (!font_path) {
  ...
  lua_pushstring(L, (const char *)font_path);
  lua_settable(L, -3);
...
__declspec(dllexport) int __cdecl shape (lua_State *L) {    
    const char * text = luaL_checkstring(L, 1);
    FT_Face face = (FT_Face)lua_touserdata(L, 2);
    ...
    hb_buffer_set_script(buf, (hb_script_t)hb_tag_from_string(script, strlen(script)));
...
__declspec(dllexport) int __cdecl luaopen_justenoughharfbuzz (lua_State *L) {
