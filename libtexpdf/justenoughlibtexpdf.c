...
__declspec(dllexport) int __cdecl pdf_init (lua_State *L) {
  pdf_rect mediabox;
...
__declspec(dllexport) int __cdecl pdf_endpage(lua_State *L) {
  ASSERT(p);
  texpdf_doc_end_page(p);  
  return 0;
};

__declspec(dllexport) int __cdecl pdf_beginpage(lua_State *L) {
  ASSERT(p);
  texpdf_doc_begin_page(p, 1,0,height);
  return 0;
}

__declspec(dllexport) int __cdecl pdf_finish(lua_State *L) {
  ASSERT(p);
...
__declspec(dllexport) int __cdecl pdf_loadfont(lua_State *L) {
  const char * filename;
  int index = 0;
  double ptsize;
  int layout_dir = 0;
...
__declspec(dllexport) int __cdecl pdf_setstring(lua_State *L) {
...
__declspec(dllexport) int __cdecl pdf_setrule(lua_State *L) {
  double x = luaL_checknumber(L, 1);
  double y = luaL_checknumber(L, 2);
...
__declspec(dllexport) int __cdecl pdf_setcolor(lua_State *L) {
...
__declspec(dllexport) int __cdecl pdf_colorpush(lua_State *L) {
  double r = luaL_checknumber(L, 1);
  double g = luaL_checknumber(L, 2);
  double b = luaL_checknumber(L, 3);
...
__declspec(dllexport) int __cdecl pdf_colorpop(lua_State *L) {
  texpdf_color_pop(p);
  return 0;
}

__declspec(dllexport) int __cdecl pdf_drawimage(lua_State *L) {
...
__declspec(dllexport) int __cdecl luaopen_justenoughlibtexpdf (lua_State *L) {
  lua_newtable(L);
